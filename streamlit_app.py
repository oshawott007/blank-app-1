...
# # We're now handling frame processing in the detection refresh section above
...
#                     stream_success = display_video_stream(camera['url'], frame_place)
#                     if stream_success:
#                         status.success("Connected - Live Stream")
#                         st.session_state.cameras[idx]["status"] = "Connected"

#                         # Get a frame for detection purposes if needed
#                         if camera["last_frame"] is None:
#                             frame_data, error = get_mjpeg_frame(camera['url'])
#                             if frame_data:
#                                 st.session_state.cameras[idx]["last_frame"] = frame_data
#                     else:
#                         status.error("Failed to start live stream")
#                 except Exception as e:
#                     status.error(f"Error displaying live stream: {str(e)}")

#                     # Fall back to static image if streaming fails
